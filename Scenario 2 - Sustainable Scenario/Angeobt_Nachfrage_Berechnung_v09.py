...
def ausfuehren():
    ...
    print('WTP combined')
    derated_wtp_calculation(wtp_compl)
    print('wtp derated')
    wtp_sorting()
    print('wtp sorted')    
...
def max_wtp_Calculation_IND(wtp_list):
   ...
   # el_cost = el_cost.drop(columns=['Energiesektor','Kode'])
    ...
    #max_wtp_list.to_csv('Data\maxWTP_Ind_Vorher.csv', index = None, sep=";", mode="a")
...
def derated_wtp_calculation(wtp_list):
   ...
   # wtp
    derated_wtp_list = pd.DataFrame(columns=['Land', 'Größe', 'Sektor', 'Technologie'])
    ...
    for t in range(2,8):
        ...
        for year in range(2010,2018):      
            derated_wtp_list_tec[year] = derated_wtp_list_compl[year]*derated_wtp_list_tec[tec]                                              
    ...
    #for year in range(2010,2018):      
    #        derated_wtp_list.drop([year], axis=1, inplace=True)    
...
# =============================================================================
def wtp_sorting():
    derated_wtp = pd.read_csv('Data/deratedWTP_2.csv', delimiter = ";")
...
def assume_industry_demand():
  ...
  #  ind_dem_assum = pd.DataFrame(columns=["Land","Sektor", "Größe"])
    ...
    for year in range(2020, 2041):
        complete_sector[year] = complete_sector[year]*complete_sector[2017]    #Multiplikation mit der Anzahl der UN pro Sektor, Land und Größe
...
def assume_household_demand():
  ...
  #  ind_dem_assum = pd.DataFrame(columns=["Land","Sektor", "Größe"])
    ...
    #complete_sector.to_excel('Data\hhdem_complete.xls', sheet_name='household demand')                   
    return hh_dem

# =============================================================================
#  Zusammenführung von Haushalts- und Industriebedarfen
# =============================================================================            
...
def combine_demand(dem_hh, dem_ind):
    ...
    i = 0.2157
    for year in range(2020,2041):
        demand_compl[year] = demand_compl[year].multiply(i)
        i = i * 1.09
